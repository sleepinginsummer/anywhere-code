...
- **浏览器端**：使用 xterm.js 渲染终端，负责本地缓冲与光标状态的持久化，并适配移动端交互。

推荐流程：后端通过本机 `wsl.exe` 连接 WSL，使用 tmux 创建/附着会话。前端连接后端的数据通道，渲染输出并在本地缓存终端状态。断线重连时先恢复本地缓存，再 attach 旧会话继续交互，从而达到“回到断开前的光标位置”的体验。
...
## 移动端适配
- 响应式布局：终端区域与会话列表自适应窄屏，支持折叠侧栏。
- 触控优化：更大的点击区域、支持长按复制/粘贴、双击选择单词。
- 软键盘处理：输入框与终端绑定，避免键盘遮挡；支持自定义快捷键（如 Ctrl/Cmd 组合）。
- 安全区域：适配 iOS 安全区与底部手势条，避免 UI 被遮挡。
...
## 错误处理
- 细分错误码：认证失败、会话不存在、会话超限、WSL 失败、tmux 异常等。
- 前端提示可恢复与不可恢复错误；超时后提示用户手动重连。

## 测试建议
- 单元测试：会话创建/删除、超限、改名、最后输入摘要逻辑。
- 集成测试：WSL/tmux 创建与附着、断线重连。
